...
function StoreHash(value, flags, ply, move, depth)
    ...
    g_hashTable[1 + bit.band(g_hashKeyLow, g_hashMask)] = HashEntry(g_hashKeyHigh, value, flags, ply, move)
end
...
function IsHashMoveValid(hashMove)
    ...
    if (pieceType == piecePawn) then
        ...
        if
            (((row == 0x90 and (g_toMove == 0)) or (row == 0x20 and g_toMove > 0)) ~=
                (bit.band(hashMove, moveflagPromotion) > 0))
         then
            -- Handle promotions
            return false
        end
...
function nextMove(mp)
    ...
    if
        ((mp.stage > 1 and candidateMove == mp.hashMove) or (mp.stage > 3 and candidateMove == mp.killer1) or
            (mp.stage > 4 and candidateMove == mp.killer2))
     then
        return nextMove(mp)
    end
...
function AllCutNode(ply, depth, beta, allowNull)
    ...
    if (GetTime() - g_startTime > g_timeout) then
        -- Time cutoff
        g_searchValid = false
        return beta - 1
    end
    ...
    if ((not g_inCheck) and allowNull and (beta > minMateBuffer) and (beta < maxMateBuffer)) then
        ...
        if
            (ply > 1 and g_baseEval >= beta - iif(ply >= 4, 2500, 0) and
                (g_pieceCount[1 + bit.bor(pieceBishop, g_toMove)] ~= 0 or
                    ...
                    g_pieceCount[1 + bit.bor(pieceQueen, g_toMove)] ~= 0))
         then
...
function Evaluate()
    ...
    if (g_pieceList[1 + bit.lshift(bit.bor(colorWhite, pieceQueen), 4)] == 0) then
        evalAdjust =
            evalAdjust + pieceSquareAdj[1 + pieceKing][1 + flipTable[1 + g_pieceList[1 + bit.lshift(pieceKing, 4)]]]
    end
...
function MakeMove(move)
    ...
    if (flags > 0) then
        if (bit.band(flags, moveflagCastleKing) > 0) then
            if (IsSquareAttackable(from + 1, otherColor) or IsSquareAttackable(from + 2, otherColor)) then
                g_moveCount = g_moveCount - 1
                return false
            end
    ...
    if (captured > 0) then
        ...
        g_baseEval =
            g_baseEval + pieceSquareAdj[1 + bit.band(captured, 0x7)][1 + iif(me > 0, flipTable[1 + epcEnd], epcEnd)]
...
function GetMoveSAN(move, validMoves)
    ...
    for i = 0, table.getn(validMoves) - 1, 1 do
        ...
        if (moveFrom ~= from and moveTo == to and (bit.band(g_board[1 + moveFrom], 0x7) == pieceType)) then
            dupe = true
...
function GenerateAllMoves(moveStack)
    ...
    if (not g_inCheck) then
        ...
        if (bit.band(castleRights, 2) > 0) then
            -- Queenside castle
            if
                (g_board[1 + (from - 1)] == pieceEmpty and g_board[1 + (from - 2)] == pieceEmpty and
                    g_board[1 + (from - 3)] == pieceEmpty)
             then
...
function MovePawnTo(moveStack, start, square)
    ...
    if ((row == 0x90) or (row == 0x20)) then
        moveStack[1 + table.getn(moveStack)] =
            GenerateMove2(start, square, bit.bor(moveflagPromotion, moveflagPromoteQueen))
        moveStack[1 + table.getn(moveStack)] =
            GenerateMove2(start, square, bit.bor(moveflagPromotion, moveflagPromoteKnight))
        moveStack[1 + table.getn(moveStack)] =
            GenerateMove2(start, square, bit.bor(moveflagPromotion, moveflagPromoteBishop))
...
function GeneratePawnMoves(moveStack, from)
    ...
    if (g_board[1 + to] == 0) then
        ...
        if
            (((bit.band(from, 0xF0) == 0x30) and color ~= colorWhite) or
                ((bit.band(from, 0xF0) == 0x80) and color == colorWhite))
         then
            to = to + inc
            if (g_board[1 + to] == 0) then
                moveStack[1 + table.getn(moveStack)] = GenerateMove(from, to)
...
function See(move)
    ...
    if
        ((bit.band(g_board[1 + (to + inc + 1)], 0xF) == bit.bor(piecePawn, them)) or
            (bit.band(g_board[1 + (to + inc - 1)], 0xF) == bit.bor(piecePawn, them)))
     then
        return false
    end
    ...
    if
        ((bit.band(g_board[1 + (to - inc + 1)], 0xF) == bit.bor(piecePawn, us)) or
            (bit.band(g_board[1 + (to - inc - 1)], 0xF) == bit.bor(piecePawn, us)))
     then
        g_board[1 + from] = fromPiece
        return true
    end
